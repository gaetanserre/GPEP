    ...
    **kwargs
    ...
    if has_bounds:
        if scaling == False:
            ...
            p = lcmaes.make_parameters_pwqb(x0, lambda_, sigma0, gp)
        else:
            gp = lcmaes.make_genopheno_pwqb_ls(lbounds, ubounds, len(ubounds))
            p = lcmaes.make_parameters_pwqb_ls(x0, lambda_, sigma0, gp, -1, 0)
    else:
        if vscaling is None:
            p = lcmaes.make_simple_parameters(x0, lambda_, sigma0)
        else:
            gp = lcmaes.make_genopheno_ls(vscaling, vshift)
            p = lcmaes.make_parameters_ls(x0, lambda_, sigma0, gp)
